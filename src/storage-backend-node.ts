...
export class NodeFileSystemStorageBackend implements StorageBackend {
    ...
    private async ensureOpened() {
        ...
        if (segments.length === 0) {
            ...
            const st = await fsp!.stat(path!.join(this.dir, this.pad6(this.activeId) + ".kvlog"));
    ...
    async get(key: string): Promise<StoredRecord | undefined> {
        ...
        const encodings: StoredRecord["encoding"][] = ["text", "json", "clone", "binary"];
        ...
        else if (encoding === "clone") value = JSON.parse(valueBuf.toString("utf8")); // Simplified for POC
    ...
    async put(record: StoredRecord): Promise<void> {
        ...
        if (record.encoding === "text") valBuf = Buffer.from(record.value as string, "utf8");
        else if (record.encoding === "json" || record.encoding === "clone") valBuf = Buffer.from(JSON.stringify(record.value), "utf8");
        else {
    ...
    private async getReadFd(fileId: number): Promise<number> {
        if (fileId === this.activeId && this.activeFd !== null) return this.activeFd;
    ...
    private fsRead(fd: number, buffer: Buffer, offset: number, length: number, position: number): Promise<number> {
        return new Promise((resolve, reject) => {
