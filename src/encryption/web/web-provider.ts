...
export class WebCryptoEncryptionProvider extends BaseEncryptionProvider {
  ...
  async initialize(): Promise<void> {
    ...
    if (typeof this.keyMaterial === "string") {
      ...
      if (this.keyMaterial.length !== 32) {
        throw new Error(
          "WebCryptoEncryptionProvider requires a 256-bit key (32 bytes)",
        );
      }
  ...
  async encrypt(plaintext: Uint8Array): Promise<Uint8Array> {
    if (!this.cryptoKey) {
      throw new Error(
        "WebCryptoEncryptionProvider not initialized. Call initialize() first.",
      );
    }
...
export class PassphraseEncryptionProvider extends BaseEncryptionProvider {
  ...
  async encrypt(plaintext: Uint8Array): Promise<Uint8Array> {
    if (!this.cryptoKey) {
      throw new Error(
        "PassphraseEncryptionProvider not initialized. Call initialize() first.",
      );
    }
    ...
    const combined = new Uint8Array(
      this.salt.length + iv.length + ciphertext.byteLength,
    );
  ...
  async decrypt(ciphertext: Uint8Array): Promise<Uint8Array> {
    ...
    try {
      ...
      throw new Error(
        `Decryption failed: ${e instanceof Error ? e.message : "Invalid passphrase or corrupted data"}`,
      );
    }
  }
