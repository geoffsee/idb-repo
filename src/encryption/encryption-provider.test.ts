...
describe("WebCryptoEncryptionProvider", () => {
  ...
  it("should initialize from a string passphrase (simple hash)", async () => {
    const stringProvider = new WebCryptoEncryptionProvider(
  ...
  it("should throw error if decrypting with wrong key", async () => {
    const provider1 = new WebCryptoEncryptionProvider(
      new Uint8Array(32).fill(0x01),
    );
    await provider1.initialize();

    const provider2 = new WebCryptoEncryptionProvider(
  ...
  it("should throw if not initialized", async () => {
    ...
    // Skip initialize()
    await expect(uninit.encrypt(testPlaintext)).rejects.toThrow(
      "not initialized",
    );
  });
  ...
  it("should validate key length if Uint8Array provided", async () => {
    ...
    await expect(badProvider.initialize()).rejects.toThrow(
      "requires a 256-bit key (32 bytes)",
    );
  });
});
...
describe("PassphraseEncryptionProvider", () => {
  ...
  const testPlaintext = new TextEncoder().encode(
    "Secret message derived from PBKDF2",
  );
  ...
  it("should use a consistent salt if provided", async () => {
    ...
    const provider1 = await PassphraseEncryptionProvider.create(
      passphrase,
      salt,
    );
    const provider2 = await PassphraseEncryptionProvider.create(
      passphrase,
      salt,
    );
