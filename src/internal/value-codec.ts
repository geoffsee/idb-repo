 ...
 */
export function normalizePutValueSync(value: KVValue): { encoding: StoredEncoding; stored: unknown } | null {
    ...
    if (value instanceof ArrayBuffer) return { encoding: "binary", stored: new Blob([value]) };
    if (isArrayBufferView(value)) return { encoding: "binary", stored: new Blob([value.buffer.slice(0) as ArrayBuffer]) };
 ...
 */
export async function normalizePutValue(value: KVValue): Promise<{ encoding: StoredEncoding; stored: unknown }> {
...
export function decodeValue(
    ...
    if (rec.encoding === "json") {
        ...
        if (want === "arrayBuffer") return new TextEncoder().encode(jsonText).buffer;
