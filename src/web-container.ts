    ...
    `);
        ...
        this.indexedDB = {
            get: async <T>(store: string, key: string) => {
                return await this.page.evaluate(async ({ store, key }) => {
            ...
            set: async <T>(store: string, key: string, value: T) => {
                await this.page.evaluate(async ({ store, key, value }) => {
                    const db = await openDB('webcontainer-db', 1, [store]);
            ...
            del: async (store: string, key: string) => {
                await this.page.evaluate(async ({ store, key }) => {
                    const db = await openDB('webcontainer-db', 1, [store]);
                    const tx = db.transaction(store, 'readwrite');
...
function reqToPromise(req: IDBRequest) { return globalThis.reqToPromise(req); }
function txDone(tx: IDBTransaction) { return globalThis.txDone(tx); }
function openDB(name: string, version: number, stores: string[]) { return globalThis.openDB(name, version, stores); }
